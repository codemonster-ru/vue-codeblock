...
const wrapToken = (tokenClass: string, value: string) =>
  `<span class="vcb__token ${tokenClass}">${value}</span>`;

const jsNumberPattern = "\\b\\d(?:[\\d_]*\\d)?(?:\\.\\d(?:[\\d_]*\\d)?)?\\b";
const cssNumberPattern =
  "\\b-?\\d(?:[\\d_]*\\d)?(?:\\.\\d(?:[\\d_]*\\d)?)?(?:px|rem|em|%|vh|vw|fr|s|ms|deg)?\\b";
...
const highlightTemplateLiteral = (value: string): string => {
  let cursor = 0;
  let html = wrapToken("vcb__token_string", escapeCodeHtml("`"));

  while (cursor < value.length - 1) {
    const interpolationStart = value.indexOf("${", cursor);
    const segmentEnd =
      interpolationStart === -1 ? value.length - 1 : interpolationStart;
    const segment = value.slice(cursor, segmentEnd);

    if (segment) {
      html += wrapToken("vcb__token_string", escapeCodeHtml(segment));
    }
  ...
  html += wrapToken("vcb__token_string", escapeCodeHtml("`"));
  return html;
};
...
const highlightDirectiveExpressionValue = (attributeValue: string) => {
  if (attributeValue.length < 2) {
    return wrapToken("vcb__token_string", attributeValue);
  }
...
const highlightMultilineAttributeSegment = (
  ...
  const highlightedInner = directive
    ...
    : wrapToken("vcb__token_string", escapeCodeHtml(value));
  const escapedQuote = quote === '"' ? "&quot;" : "&#39;";
...
const highlightMarkupOpeningStart = (line: string) => {
  ...
  const tagTokenClass = isComponentTagName(tagName)
...
const highlightMarkupTag = (rawTag: string) => {
  if (rawTag.startsWith("<!--") && rawTag.endsWith("-->")) {
    return wrapToken("vcb__token_comment", escapeCodeHtml(rawTag));
  }

  const closing = rawTag.startsWith("</");
  const selfClosing = rawTag.endsWith("/>");
  const inner = rawTag.slice(
    closing ? 2 : 1,
    rawTag.length - (selfClosing ? 2 : 1),
  );
...
const highlightJson = (line: string) => {
  ...
  return escaped
    .replaceAll(/"(?:[^"\\]|\\.)*"(?=\s*:)?/g, (match) =>
      wrapToken("vcb__token_string", match),
    )
    .replaceAll(/\b(true|false|null)\b/g, (match) =>
      wrapToken("vcb__token_keyword", match),
    )
    .replaceAll(
      /\b-?\d(?:[\d_]*\d)?(?:\.\d(?:[\d_]*\d)?)?(?:[eE][+-]?\d(?:[\d_]*\d)?)?\b/g,
      (match) => wrapToken("vcb__token_number", match),
    );
};

const readPreviousNonWhitespace = (
  value: string,
  startIndex: number,
): string => {
...
const classifyJsIdentifier = (
  ...
  if (nextCharacter === "(") {
    return "vcb__token_function";
  }

  if (
    /^[A-Z]/.test(identifier) ||
    ["import", "from", "new", "class", "extends"].includes(previousWord)
  ) {
    return "vcb__token_identifier";
  }

  return "vcb__token_variable";
};
...
const highlightMarkupLike = (line: string) => {
  ...
  while (cursor < line.length) {
    ...
    html += highlightInterpolations(
      escapeCodeHtml(line.slice(cursor, tagStart)),
    );
...
const highlightBash = (line: string) => {
  ...
  const highlightBashArguments = (value: string) => {
    const tokenPattern =
      /(\s+)|((["'])(?:\\.|(?!\3).)*\3)|(\$[A-Za-z_][A-Za-z0-9_]*)|(@[A-Za-z0-9._/-]+|[A-Za-z0-9._-]+\/[A-Za-z0-9._/@-]+)|(-{1,2}[A-Za-z][\w-]*)(?:=(\S+))?|([^\s]+)/g;
    let cursor = 0;
    let html = "";
  ...
  const packageManagerMatch = line.match(
  ...
  if (packageManagerMatch) {
    ...
    return `${escapeCodeHtml(indentation)}${wrapToken(
      ...
      "vcb__token_function",
      escapeCodeHtml(subcommand),
    )}${escapeCodeHtml(spaceAfterSubcommand)}${highlightBashArguments(rest)}`;
  }
...
const highlightCssLike = (line: string) => {
  ...
  if (propertyMatch) {
    const [, indentation, propertyName, separator, propertyValue] =
      propertyMatch;
    return `${escapeCodeHtml(indentation)}${wrapToken(
      "vcb__token_property",
...
export const highlightCodeLine = (
  ...
  if (
    normalizedLanguage.includes("bash") ||
    normalizedLanguage.includes("shell") ||
...
const highlightMarkupBlock = (code: string, highlight: boolean): string => {
  ...
  let multilineAttributeState: { quote: '"' | "'"; directive: boolean } | null =
    null;
  ...
  return lines
    .map((rawLine) => {
      ...
      if (/^&lt;script\b/i.test(trimmed)) {
        ...
        return hasTagEnd
          ? highlightMarkupLike(line)
          : highlightMarkupOpeningStart(line);
      }
      ...
      if (isInsideOpeningTag) {
        ...
        const multilineAttributeMatch = line.match(
          /^(\s+)([:@#]?[\w-]+(?::[\w-]+)?)(\s*=\s*)(["'])([\s\S]*)$/,
        );
        if (multilineAttributeMatch) {
          const [, whitespace, attributeName, equals, quote, initialValue] =
            multilineAttributeMatch;
